$(function () {
	...
	$("#buscar").on("keyup", function (event){
		...
		$.post("pagination.php?page=1&query=" + query, function (data) {
			var htm = "";
    ...
    $.post("pagination.php?page=1&query=", function (data) {
        var htm = "";
        var resp = jQuery.parseJSON(data);
        if (resp['rec'].length > 0) {
            for (var i = 0; i < resp['rec'].length; i++) {
    ...
    $("#append-pagination").on("click", ".pagination a", function (e) {
        ...
        $.post("pagination.php?page=" + pageNum + "&query=" + query, function (data) {
            var htm = "";
            var resp = jQuery.parseJSON(data);
            if (resp['rec'].length > 0) {
                for (var i = 0; i < resp['rec'].length; i++) {
